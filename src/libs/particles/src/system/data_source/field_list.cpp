...
void FieldList::CreateEmptyBoolField(const char *Name, bool def_value)
{
    ...
    pDesc.HashValue = ffi_hash_ignore_case(pDesc.Name.c_str());
...
{
    const uint32_t SearchHash = ffi_hash_ignore_case(AttrName);
    for (uint32_t n = 0; n < Fields.size(); n++)
    {
        if (Fields[n].Type == FIELD_COLOR)
        {
            ...
            {
                if (rust::string::iEquals(Fields[n].Name, AttrName))
                {
...
{
    const uint32_t SearchHash = ffi_hash_ignore_case(Name);

    for (uint32_t n = 0; n < Fields.size(); n++)
    {
        if (SearchHash == Fields[n].HashValue)
        {
            if (rust::string::iEquals(Fields[n].Name, Name))
            {
                return &Fields[n];
            }
        }
