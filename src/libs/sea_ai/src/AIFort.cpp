...
bool AIFort::ScanFortForCannons(AI_FORT *pFort, char *pModelsDir, char *pLocatorsName) const
{
    ...
    while (pNode = pModel->GetNode(dwIdx))
    {
        ...
        for (long i = 0; i < info.nlabels; i++)
        {
            ...
            if (storm::iEquals(std::string_view(label.name), "cannon", 6) && pFort->dwCannonType != -1)
            {
                pFort->aCannons.push_back(AICannon{});
                pCannon = &pFort->aCannons.back();
                // pCannon = &pFort->aCannons[pFort->aCannons.Add()];
            }
            if (storm::iEquals(std::string_view(label.name), "culverin", 8) && pFort->dwCulverinType != -1)
            {
                pFort->aCulverins.push_back(AICannon{});
                pCannon = &pFort->aCulverins.back();
                // pCannon = &pFort->aCulverins[pFort->aCulverins.Add()];
            }
            if (storm::iEquals(std::string_view(label.name), "mortar", 6) && pFort->dwMortarType != -1)
            {
