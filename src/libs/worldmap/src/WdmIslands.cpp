...
WdmIslands::WdmIslands()
{
    ...
    for (int32_t i = 0; i < ginfo.nlabels; i++)
    {
        ...
        if (!label.group_name || !label.group_name[0])
            continue;
        if (storm::iEquals(std::string_view(label.group_name), "locators"))
        {
            if (storm::iEquals(std::string_view(label.name), "min"))
            {
                vmn = ((CMatrix *)label.m)->Pos();
                isMin = true;
            }
            if (storm::iEquals(std::string_view(label.name), "max"))
            {
                vmx = ((CMatrix *)label.m)->Pos();
                isMax = true;
            }
        }
        else if (storm::iEquals(std::string_view(label.group_name), "merchant"))
        {
            merchants.push_back(((CMatrix *)label.m)->Pos());
        }
        else if (label.name && label.name[0] && storm::iEquals(std::string_view(label.group_name), "quests"))
        {
    ...
    for (int32_t i = 0; i < ginfo.nlabels; i++)
    {
        ...
        if (!label.name || !label.name[0])
            continue;
        if (!storm::iEquals(std::string_view(label.group_name), "islands"))
            continue;
        // Skip if added
        int32_t j;
...
void WdmIslands::SetIslandsData(ATTRIBUTES *apnt, bool isChange)
{
    ...
    for (uint32_t i = 0; i < numAttr; i++)
    {
        ...
        {
            if (!storm::iEquals(labels[index].locatorName, std::string_view(locator)))
            {
                if (LabelsFindLocator(locator, pos))
                {
                    labels[index].locatorName = locator;
...
int32_t WdmIslands::LabelsFind(const char *id, uint32_t hash)
{
    ...
    for (i = labelsEntry[i]; i >= 0; i = labels[i].next)
    {
        ...
        {
            if (storm::iEquals(labels[i].id, std::string_view(id)))
                return i;
        }
    }
    return -1;
...
bool WdmIslands::LabelsFindLocator(const char *name, CVECTOR &pos) const
{
    ...
    for (int32_t i = 0; i < ginfo.nlabels; i++)
    {
        ...
        if (!label.group_name || !label.group_name[0])
            continue;
        if (!storm::iEquals(std::string_view(label.group_name), "labels"))
            continue;
        // if(_stricmp(label.group_name, "geometry") != 0) continue;
        if (storm::iEquals(std::string_view(label.name), std::string_view(name)))
        {
...
int32_t WdmIslands::LabelsAddFont(const char *name)
{
    ...
    {
        if (storm::iEquals(fonts[i].name, std::string_view(name)))
        {
            return fonts[i].id;
        }
    }
