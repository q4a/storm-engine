...
uint32_t getSteamEnabled(VS_STACK *pS)
{
    ...
    pV->Set(SteamApi::getInstance().isSteamEnabled());
...
uint32_t getDlcEnabled(VS_STACK *pS)
{
    ...
    const int32_t dlc_active = SteamApi::getInstance().isDLCActive(dlc);
    pV = pS->Push();
    if (!pV)
    {
        return IFUNCRESULT_FAILED;
...
uint32_t getDlcData(VS_STACK *pS)
{
    ...
    const int32_t dlc_data = SteamApi::getInstance().getDLCDataByIndex(dlc);
    pV = pS->Push();
    if (!pV)
    {
        return IFUNCRESULT_FAILED;
