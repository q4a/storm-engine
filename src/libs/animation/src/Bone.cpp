...
void Bone::SetAngles(const Quaternion *aArray, long numAng)
{
...
inline void Bone::GetFrame(long f, Quaternion &qt)
{
...
void Bone::BlendFrame(long frame, float kBlend, Quaternion &res)
{
    if (numFrames <= 0)
        return;
    const auto f0 = frame;
