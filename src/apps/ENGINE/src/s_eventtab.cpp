                                     ...
                                     bool bStatic)
{
    ...
    for (uint32_t n = 0; n < Event_num[ti]; n++)
    {
        ...
        {
            if (!storm::iEquals(std::string_view(event_name), std::string_view(pTable[ti][n].name)))
                continue;
...
bool S_EVENTTAB::DelEventHandler(const char *event_name, uint32_t func_code)
{
    ...
    {
        if (pTable[ti][n].hash == hash)
            if (storm::iEquals(std::string_view(pTable[ti][n].name), std::string_view(event_name)))
            {
